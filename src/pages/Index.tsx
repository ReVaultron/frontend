...
const Dashboard = () => {
  ...
  const walletValueUSD = useMemo(() => {
    ...
    const hbarAmount = parseFloat(formatUnits(walletBalance.value, 8));
    const usdValue = hbarAmount * hbarPrice.priceUSD;
    ...
    return (
      <div className="flex-1 p-6 space-y-6 max-w-[1600px] mx-auto">
        <Alert variant="destructive">
          <AlertDescription>
            Unable to fetch price data. Please refresh the page or try again later.
          </AlertDescription>
        </Alert>
      </div>
    );
    ...
    <div className="flex-1 p-6 space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <MetricCard
          ...
          subtitle={hbarPrice ? `±$${hbarPrice.confidenceUSD.toFixed(4)}` : "Loading..."}
          isLive={!!hbarPrice}
      ...
      <div className="flex items-center justify-between pt-6 border-t border-border">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className={`h-2 w-2 rounded-full ${hbarPrice ? "bg-green-500 animate-pulse" : "bg-orange-500"}`} />
            <span className="text-sm text-muted-foreground">
              Price Feed:{" "}
              <span className={hbarPrice ? "text-green-500" : "text-orange-500"}>
          ...
          <div className="h-4 w-px bg-border" />
          <span className="text-sm text-muted-foreground">
            Source: <span className="font-medium text-foreground">Pyth Hermes</span>
          </span>
        </div>
        {priceLastUpdate && (
